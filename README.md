...
> **Status:** under active construction, built in the background. It is fully **isolated** — own `gas_event` post type, temporary `/gas-events/` URL slug, `_gas_*` meta, `gas_events_*` options. It does **not** touch MEC, the MU plugins, the theme, or any production data. The live site keeps running unchanged until a single deliberate cutover. See [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md).

## What it does (target)

- Owns event data as a clean `gas_event` model (single start/end datetime, status, cover, single venue/organizer — no MEC cruft).
...
See `docs/ARCHITECTURE.md` §11. P1 (this commit) = plugin scaffold, `gas_event` model, settings, Event Details meta box, and the All Events list.
