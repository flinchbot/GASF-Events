...
**Decisions locked by user:** Clean `gas_event` data model · Native FB importer (drop 3rd-party) · Spec-first
...
- **Own namespace only.** `gas_event` post type, temporary `/gas-events/` rewrite slug, `_gas_*` meta, `gas_events_*` options. The plugin **never reads or writes** `mec-events`, `mec_*` meta, or the `_4UX_mec_*` tables — **except read-only** during migration (§8).
- **No collisions / no overrides.** It registers nothing that touches MEC or the MU modules: no `[MEC]`/`[mec_*]` shortcodes, no `single-mec-events.php` template hook, no filters on MEC output. Its `template_include` is guarded to `is_singular('gas_event')` only; its menu/CPT are separate.
- **Aliases deferred to cutover.** The `[mec_*]→[gas_*]` back-compat aliases (§5.3) are **NOT** registered during the parallel run — the live MU modules still define those tags and a second declaration would fatal. Parallel run exposes `[gas_events]` only.
...
### Read side — LOW coupling (raw postmeta; repoint to `_gas_*`)
| Component | Role | Disposition |
|---|---|---|
| `templates/single-mec-events.php` (Module G) | Branded single-event page | **PORT** to native single template, read `_gas_*` |
| `05-yoast-mec-schema.php` | JSON-LD Event schema injector | **PORT** into plugin, build schema from `_gas_*` |
...
|---|---|---|
| `_gas_start` | string `Y-m-d H:i:s` (site-local) | Canonical start. Replaces `mec_start_date` + `mec_start_time_hour/minutes/ampm` |
| `_gas_end` | string `Y-m-d H:i:s` (site-local) | Canonical end. Replaces `mec_end_date` + `mec_end_time_*` |
| `_gas_start_ts` | int (UTC unix) | DST-correct sort/range key (via `wp_timezone()`). Replaces `mec_start_day_seconds` math |
| `_gas_end_ts` | int (UTC unix) | Range key |
| `_gas_all_day` | bool (`0/1`) | Replaces `mec_allday` |
| `_gas_hide_time` | bool | Replaces `mec_hide_time` |
| `_gas_hide_end` | bool | Replaces `mec_hide_end_time` |
| `_gas_status` | enum: `''`,`cancelled`,`postponed`,`sold_out`,`online_only` | Replaces `mec_event_status` |
| `_gas_status_reason` | string | Optional cancellation note |
| `_gas_online_link` | url | For `online_only` |
| `_gas_more_info_url` / `_gas_more_info_title` | string | Replaces `mec_more_info*` |
| `_gas_cost` | decimal | Only set when > 0; drives schema `offers` |
| `_gas_source` | enum: `manual`,`facebook`,`eventbrite` | **Provenance.** Manual events have no `_gas_fb_event_id`; the FB importer only ever touches `_gas_source = facebook`. Drives the admin "source" badge |
| `_gas_fb_event_id` | string | FB dedup key. Replaces `mec_advimp_facebook_event_id` |
| `_gas_fb_account` | string | Which FB page sourced it (when >1 page) |
| `_gas_fb_cover_id` | string | Last-synced FB cover id/hash (change detection) |
| `_gas_fb_missing` | int (0–2) | Consecutive not-found counter → auto-draft |
| `_gas_sync_locked` | bool | On an FB event, the admin "pin" — sync skips the whole event (see §4.6) |
| `_gas_series_id` | string | Groups occurrences of a recurring event (manual **or** FB) for series-level edit/delete (see §4.7) |
| `_gas_series_role` | enum: `single`,`master`,`occurrence` | Series membership; `master` holds the recurrence template |
| `_gas_eventbrite_id` | string | Eventbrite event id once published (also the "already published" check) |
| `_gas_eventbrite_url` | url | Public Eventbrite listing (shown in the All Events column) |
| `_gas_eventbrite_status` | enum: `''`,`published`,`error` (+ `_gas_eventbrite_synced_at`) | Last publish/update outcome |
| `_gas_views` | int (+ `_gas_views_daily` roll-up) | Lifetime page views; web/kiosk split (§4.5.1) |
| `_gas_cover_sha1` | string (**on attachment**) | SHA1 of cover file for master-attachment dedup |

**Timezone rule:** all display/derivation uses `wp_timezone()` (site tz, currently America/New_York). `_gas_*_ts` are the only UTC values; they exist purely for fast, DST-safe range queries and sorting. This eliminates MEC's awkward `day_seconds` scheme.
...
```
gas_events_venue       = { name, street, city, state, zip, country, lat, lng, hide_map (bool) }
gas_events_organizer   = { name, url }                  // → schema Event.organizer {name, url}
gas_events_default_image = attachment_id | url          // site-wide fallback cover
```
- **Venue default:** *German American Society, 8098 66th Street North, Pinellas Park, FL 33781*. Optional per-event override meta `_gas_venue_override` (rare).
- **Organizer default:** *German American Society* + the club URL. Optional per-event override meta `_gas_organizer_override`.
...
Because `gas_event` is a normal CPT, WordPress gives **Add New / Edit / Trash / Delete** for free in the standard list table. On top of that we ship a single clean **"Event Details" meta box** — designed for non-technical maintainers (the CLAUDE.md admin-simplicity constraint), not MEC's wall of tabs:
- **Title** + **description** (standard WP editor).
- **Start / End** — native date + time pickers (one row each; an "All day" checkbox hides the time fields). Writes `_gas_start/_gas_end` and recomputes `_gas_*_ts` on save via `wp_timezone()`.
...
- **Validation:** end ≥ start; warn on save if a manual event collides with an identical FB event. Capability `edit_gas_events` maps to the Maintainer/Admin role split.
...
The standard CPT list table at `edit.php?post_type=gas_event` is the **All Events** view, enhanced:
- **Columns:** cover thumbnail · title · **Added** (post date — when it entered the system, sortable) · **Event date** (`_gas_start`, sortable; shows end too when multi-day) · **Recurring** (✓ when part of a series, §4.7) · Status · Source (Manual / Facebook) · **Views** (lifetime page views, §4.5.1) · **Eventbrite** (published badge + link, or "—").
- **Filters / search:**
...
- **Bulk Actions** (multiselect rows → apply):
  | Action | Behavior |
  |---|---|
  | **Publish to Eventbrite** | Queues selected events to the in-core Eventbrite publisher (§7.1); writes `_gas_eventbrite_*`. Already-published events **update** their listing (full three-way sync) |
...
- **Counting:** a tiny JS beacon on the single-event page calls a REST endpoint that increments `_gas_views` (and a daily roll-up for trends). **Beacon, not server-side PHP increment**, because Cloudflare/SpeedyCache serve cached HTML — a server counter would miss cached hits. Bots and logged-in admins are excluded.
...
- The importer **only ever reads/writes events where `_gas_source = facebook`** (identified by `_gas_fb_event_id`). Manual events are invisible to it — never updated, never auto-drafted.
...
- A single boolean `_gas_sync_locked`. When set, the importer **skips the event entirely** — no field updates, **and** it's exempt from FB-disappeared auto-draft (once you own it, FB removing its copy shouldn't unpublish yours). Mental model: *"this event is now mine."* One field of state, trivial to reason about, fits the admin-simplicity constraint.
- **Auto-pin on edit (included):** when a maintainer edits any synced field of an FB event in wp-admin, auto-set `_gas_sync_locked` and show a dismissible notice — *"This event is now manually managed; Facebook updates are paused. [Resume syncing]."* This makes the behavior discoverable instead of a hidden gotcha where edits silently revert. "Resume syncing" clears the pin and the next sync re-adopts FB's values.
- **Why not per-field in v1:** per-field is coarser to *not* do but finer to build. The good version isn't per-field checkboxes (UI clutter) — it's **automatic dirty-tracking**: snapshot each field's last-synced FB value (`_gas_fb_snapshot`), and on sync only update fields still matching their snapshot, so *"whatever you edited stays edited, everything else keeps syncing."* That's the **v2 upgrade**; the data model is shaped so it can layer on later (add `_gas_fb_snapshot` + treat `_gas_sync_locked` as the "all fields" case) **without migration**.
...
**Model:** a series shares a `_gas_series_id`; a hidden/`master` post holds the recurrence template (pattern + image + description), and each occurrence is a normal `gas_event` (`_gas_series_role = occurrence`). Standalone events are `single`.
...
- **Edit this & future** (re-stamp forward occurrences from the master), **edit just this one** (detaches it: `_gas_series_role` stays `occurrence` but pinned from series edits), **delete this one**, **delete the series**.
- A list-table "Series" filter/column so 52 Euchre nights collapse to one manageable group.

**FB recurring unification:** Facebook recurring events (the `event_times[]` expansion in §6.2) use the **same** `_gas_series_id` mechanism — one FB recurring event → one series of flat occurrences sharing a cover. So manual and FB recurrence are one concept, one set of bulk actions, one mental model. (This subsumes today's `gasf_mec_recurring_parent` hack.)
...
```
[gas_events view="month"]   → month grid (pages 6058, 8647)
[gas_events view="list"]    → upcoming list (page 13415)
[gas_events view="tile"]    → signage tiles (page 12730)
```
...
- **Querying:** all views query `gas_event` by `_gas_start_ts >= now` (or month bounds), ordered by `_gas_start_ts` (numeric `meta_query`).

### 5.2 Single-event page
Port Module G (`single-mec-events.php`) to a plugin-owned `template_include` (priority 100, guarded by `is_singular('gas_event')`), reading `_gas_*`. Keeps: branded layout, status banner (cancelled/postponed), inline CSS (plugin dir may be outside web root). Survives theme reinstalls. Add-to-calendar buttons come from the native helper (§5.6).

### 5.3 Custom shortcodes — full rename + aliases ("all in")
Rename to the `gas_*` namespace with internals reading `gas_event`/`_gas_*`:
`[mec_upcoming_dates]→[gas_upcoming_dates]`, `[german_dinner_events]→[gas_dinner_events]`, `[bayern_match_events]→[gas_bayern_events]`, `[world_cup_schedule]→[gas_world_cup_schedule]`. **Old tags are registered as back-compat aliases** so existing page content keeps working; page content is migrated to the new tags and aliases retired after a grace period. No `mec_*` names survive in new code.
...
A single helper `gas_events_add_to_calendar($event)` renders a **dropdown/button group** so any visitor can add an event to their own calendar. All targets are computed from `_gas_start/_gas_end` + `_gas_*_ts` (UTC-exact, DST-correct) — no per-template URL hand-building.
...
| **Download .ics** | universal fallback (Apple, Thunderbird, any client) — per-event `/?gas-events-ical=1&event=<id>` |
...
Each `gas_event` is its own URL (`/events/<slug>/`, preserved through migration) and ships the full SEO surface. This is the single source of event structured data — **MEC's native schema stays off** (it created the duplicate-schema problem).

**schema.org `Event` JSON-LD** (drives Google's event rich results), built from `_gas_*`:
...
| `eventStatus` | mapped from `_gas_status` → `EventScheduled` / `EventCancelled` / `EventPostponed` / `EventMovedOnline` |
| `eventAttendanceMode` | Offline (club) or Online (`online_only`) |
| `location` | `Place` (club name + full address + geo) or `VirtualLocation` (uses `_gas_online_link`) |
| **`image`** | event cover, ideally multiple ratios (16:9 / 4:3 / 1:1); **falls back to `gas_events_default_image` when the event has no cover so `image` is never absent** (§4.3) |
| **`organizer`** | `{ @type: Organization, name, url }` from `gas_events_organizer` (per-event override allowed) — **name + URL always emitted** |
| **`eventStatus`** | always emitted; `_gas_status` → `EventScheduled` / `EventCancelled` / `EventPostponed` / `EventMovedOnline` |
| **`offers`** | emitted when `_gas_cost > 0` — `{ price, priceCurrency: USD, availability, url, validFrom }` |
| `url` | canonical event URL |

> **Non-negotiable (per requirement):** every Event block carries **organizer name, organizer URL, an image (cover or default), offers (when priced), and eventStatus**. These are wired from the settings in §4.3 + `_gas_*` so they're present on all 860+ events, not just hand-made ones.
...
### 6.1 Trigger
- **Single** WP-cron hook `gas_events_sync` on a sane interval (proposed **every 15 min**; configurable). Registered once; **no per-pageload rescheduling** (the bug Module A fixed). Optionally backed by a real cPanel cron hitting `wp cron event run gas_events_sync` for reliability on a low-traffic site.
- Manual "Sync now" + per-event `gas_events_fb_refresh($post_id, $dry_run)` for testing.
...
- **Recurring events:** expand `event_times[]` into **one flat `gas_event` per occurrence** (keyed `_gas_fb_event_id = <fb_id>@<occurrence_start>`). This is already how the data looks today (876 flat events) — we just drop MEC's custom_days/`reschedule()` machinery.
- **Time window:** import upcoming + a small past grace window; ignore far-future noise (ports Module C's intent, natively).

### 6.3 Credentials
- New option `gas_events_fb_accounts` = `[{ id, label, page_id, access_token, expire_at }]`. One-time import the existing token(s) from `mec_advimp_auth_facebook` (array keyed per page) during migration. **Long-lived page tokens; document the refresh procedure** (token longevity is the single biggest ongoing fragility — unchanged from today).

### 6.4 Upsert (FB = source of truth)
For each fetched occurrence, find existing `gas_event` by `_gas_fb_event_id`:
- **Create** if absent; **update** title/description/start/end/cover if changed.
- Writes `_gas_start/_gas_end/_gas_*_ts/_gas_all_day` directly — no MEC table, no schedule lib.
- **Cover sync + dedup:** port H's proven pattern — sideload cover, SHA1 the file, reuse an existing **master attachment** (`_gas_cover_sha1`) instead of duplicating; self-heal missing files; orphan-cleanup guarded against shared-file deletion. (This is what keeps 175 Biergartens sharing one image — must be preserved.)
- **Deletion:** FB not-found for 2 consecutive cycles → set post to `draft` (ports H + the "deleted FB events auto-unpublish" rule). Avoids the MEC **trash-limbo** dedup bug entirely because dedup is by `_gas_fb_event_id` and we draft (not trash).
...
- No SQL dedup sweep (Module E obsolete) — upsert is deterministic by `_gas_fb_event_id`.
...
- **REST:** `GET /wp-json/gas-events/v1/events?from=&to=&limit=&updated_since=` → normalized JSON (id, title, slug, url, start, end, all_day, status, image, organizer, series_id, description). `updated_since` enables **incremental** kiosk sync. Cache-friendly. *(Lock this JSON contract before the kiosk Phase 3 is built — see §10.)*
- **iCal — per-event:** `/?gas-events-ical=1&event=<id>` → powers the Apple/"Download .ics" add-to-calendar (§5.6).
- **iCal — whole-calendar subscription:** `/?gas-events-ical=1` (optionally `webcal://…`) → a **subscribable feed** members add to their phone once and it stays updated. Replaces MEC's `?mec-ical-feed=1`. (In-core Google/Eventbrite syndication reads events directly, not via this feed — §7.1.)
...
- **Propagation:** any change to a `gas_event` that has a `_gas_eventbrite_id` queues an **Eventbrite update** — whether the change came from the **FB importer** (FB edited the event → WP updated, §6) or a **manual WP edit**. So the chain is **FB → WP → EB**, end to end, no manual re-publish needed.
...
- **Deletion / cancellation:** event trashed or drafted → linked EB listing **unpublished**; `_gas_status = cancelled` → EB listing **cancelled**. (FB-vanished → WP draft → EB unpublish flows automatically.)
- **Pinned events (`_gas_sync_locked`):** the pin blocks **FB → WP** (you own the content), but **WP → EB still mirrors** whatever WP currently holds — EB always reflects WP.
- **Caveat (surfaced, not silent):** Eventbrite restricts editing a listing once tickets have sold; the publisher reports that state in `_gas_eventbrite_status = error` + the All Events column rather than failing quietly. Rare for this club.
...
1. Register `gas_event` initially with a **temporary slug** (`/gas-events/`) so the new plugin runs **in parallel** with live MEC for validation.
2. **Migration script** (idempotent, dry-run first):
   - For each `mec-events` post: derive `_gas_start/_gas_end` from `mec_start_date` + `mec_*_time_hour/minutes/ampm` (DST-correct via `wp_timezone()`); compute `_gas_*_ts`; map `mec_allday/mec_hide_time/mec_hide_end_time/mec_event_status/mec_more_info*/mec_cost/mec_advimp_facebook_event_id` → `_gas_*`.
...
4. **Cutover** (single maintenance window):
   - Convert post_type `mec-events` → `gas_event` (preserves IDs/slugs/dates/thumbnails).
   - Switch `gas_event` rewrite slug to **`events`**; `flush_rewrite_rules()`.
   - Swap the 4 calendar pages' shortcodes to `[gas_events ...]`; repoint the 5 custom shortcodes (already deployed, gated).
   - Deactivate MEC + `mec-advanced-importer`; disable Module A–F/H gates.
   - Enable native `gas_events_sync` cron.
...
**Rollback (within soak window):** `mec_*` meta + MEC tables still intact → reactivate MEC + importer, disable native cron, revert `gas_event` slug + the 4 pages' shortcodes. Fast and complete because nothing destructive happens until decommission.
...
| Event-table growth (keep-all + hourly FB) → thousands over years | MED | Index `_gas_start_ts`; date-bound all grid/list queries; cache views |
...
- **Naming:** full `gas_*` rename of meta and shortcodes, with back-compat aliases for old tags retired after a grace period (§5.3).
...
- **Admin editor:** maintainer-friendly add/edit/delete meta box; manual events shielded from FB sync via `_gas_source` (§4.5–4.6).
- **Sync pin:** **whole-event** for v1 (`_gas_sync_locked`) + auto-pin-on-edit + deletion exemption; automatic per-field dirty-tracking reserved as a no-migration v2 (§4.6).
...
- **Recurring events:** first-class **flat-but-grouped series** (`_gas_series_id`) with a "Repeats" generator + series-level edit/delete; FB recurrence uses the same mechanism (§4.7).
- **Default image:** site-wide fallback cover (`gas_events_default_image`) used in display **and** schema so no event is image-less (§4.3).
- **Organizer:** global setting (name + URL) with per-event override (§4.3).
- **Whole-calendar subscribe feed:** offered (`webcal://`), §7.
- **Retention:** keep all past events (history + SEO); index `_gas_start_ts` (§10 risk row).
...
- **Per-event view stats:** native cache-safe beacon counter (`_gas_views`), web/kiosk split, surfaced as a Views column + "Stats" row action (§4.5.1).
...
1. **P1 — Plugin scaffold + `gas_event` model + admin editor** (CPT, maintainer-friendly Event Details meta box with date/time pickers + status; venue/organizer/default-image settings; list-table columns/filters, manual add/edit/delete, role caps, `_gas_source` provenance — §4.3/4.5/4.6).
2. **P2 — Recurring series** ("Repeats" generator, `_gas_series_id`, series-level edit/delete, list-table grouping — §4.7).
3. **P3 — Migration script** (dry-run → parallel-run copies under temp slug; validation harness; 79-rich-event spot-check; index `_gas_start_ts`).
...
### Appendix A — MEC → `gas_event` meta crosswalk (for the migration script)
```
mec_start_date + mec_start_time_hour/minutes/ampm   → _gas_start (Y-m-d H:i:s) + _gas_start_ts (UTC)
mec_end_date   + mec_end_time_hour/minutes/ampm     → _gas_end   + _gas_end_ts
mec_allday                                          → _gas_all_day
mec_hide_time / mec_hide_end_time                   → _gas_hide_time / _gas_hide_end
mec_event_status                                    → _gas_status (+ _gas_status_reason, _gas_online_link)
mec_more_info / mec_more_info_title                 → _gas_more_info_url / _gas_more_info_title
mec_cost (if > 0)                                   → _gas_cost
mec_advimp_facebook_event_id                        → _gas_fb_event_id
